...
public class Application {
	...
	public static void main(String[] args) {
		...
		if (args.length == 3 || args.length == 1) {
			...
			while (true) {
				System.out.print("Enter operation to perform (i.e. add, subtract, multiply, divide, factorial, exponentiation, square root, natural logarithm, base-10 logarithm, sin, cos, tan, permutation) type exit to exit program: ");
	...
	public static double handle(String operation) {
		 ...
		 * Handles the logic of deciding which operation to invoke and asking for user input for the number(s).
		 */
		Scanner scan = new Scanner(System.in);
		...
		case "divide":
			...
			if (div2 == 0) { //Logic to handle divide by zero, throws IllegalArgumentException() to handle it.
		...
		case "factorial":
			...
			return factorial((int)factorial); //thus factorial is casted to int before being sent to the factorial() method.
		case "exponentiation":
		...
		case "sin":
			...
			if (units.equals("radians")) { //Decides whether the angle is in radians/degrees based on user input.
				...
				double angleRad = Math.toRadians(angle); //converts to radians if the angle is said to be in degrees.
				return sin(angleRad);
			} else {
				System.out.println(units);
				throw new IllegalArgumentException("units not properly specified."); //exception thrown if the user does not say radians nor degrees.
			}
		case "cos":
		...
		case "permutation":
			...
			if (totalitem < 0 || totalitem > 100 || select > totalitem || select < 0) { //handles incorrect total or select input.
	...
	private static long factorialHelper(int originalNum, int num) {
		 ...
		 * The progress bar never reaches 100% because (originalNum - 1) / originalNum is always less than 1.
	...
	public static int permutation(int total, int select) {
		if (select <= 0) {
			...
			return total * permutation(total - 1, select - 1); //recursively multiplies with the next lowest number until select <= 0.
		}
	}
	...
	public static int permute_loop(int total, int select) {
        int result = total;
        for (; select > 1; select--) {//select is already initialized, runs until select == 1 as we already have result equal to total at the start.
